...
import {
  listenStudents, listenSessions, listenInvoices, addInvoice, updateInvoice
} from '../lib/db'
...
export default function Invoices() {
  ...
  const rows = useMemo(() => {
    return students.map((student) => {
      ...
      const preview = buildMonthlyInvoice(student, sessions, year, month)
      return { student, existing, preview }
    }).filter((r) => r.existing || r.preview.sessionCount > 0)
  }, [students, sessions, invoices, year, month])
  ...
  const handleMarkPaid = async ({ paidDate, paidMethod }) => {
    await updateInvoice(markingPaid.id, { status: 'paid', paidDate, paidMethod })
    setMarkingPaid(null)
    setViewing(null)
  }
  ...
  return (
    <>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      {active && (
                        <>
                          ...
                          <button className="btn btn-outline btn-sm" onClick={() => generate(row)}>Refresh</button>
                          {active.status !== 'paid' && (
      ...
      {viewing && (
        <InvoiceView invoice={viewing} onClose={() => setViewing(null)} onMarkPaid={setMarkingPaid} />
      )}
      {markingPaid && (
